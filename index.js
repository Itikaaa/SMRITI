...
var patientProfiles = mysqlTable(
  "patient_profiles",
  {
    id: int("id").autoincrement().primaryKey(),
...
var storedFiles = mysqlTable("stored_files", {
  id: int("id").autoincrement().primaryKey(),
  caregiverId: int("caregiverId").notNull(),
...
async function updateStoredFile(caregiverId, id, patch) {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
...
async function removeStoredFile(caregiverId, id) {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
...
var uploadPurposes = ["memory-photo", "voice-note", "care-document", "other"];
function sanitizeFilename(filename) {
  return filename.trim().replace(/[^a-zA-Z0-9._-]/g, "-").slice(0, 120) || "upload";
}
var profileInput = z2.object({
  preferredName: z2.string().trim().min(1).max(120),
  age: z2.number().int().min(1).max(120).nullable().optional(),
  background: z2.string().max(6e3).nullable().optional(),
  language: z2.string().trim().min(1).max(40),
  childrenNames: z2.string().max(2e3).nullable().optional(),
  favouriteThings: z2.string().max(2e3).nullable().optional()
});
var appRouter = router({
  ...
  profile: router({
    ...
    save: protectedProcedure.input(profileInput).mutation(
      ({ ctx, input }) => upsertPatientProfile({
  ...
  files: router({
    ...
    upload: protectedProcedure.input(
      z2.object({
        ...
        sizeBytes: z2.number().int().positive().max(MAX_UPLOAD_BYTES),
        dataBase64: z2.string().min(1),
        purpose: z2.enum(uploadPurposes).default("other"),
        patientProfileId: z2.number().int().positive().nullable().optional(),
        consentStatus: z2.enum(["pending", "confirmed", "declined"]).default("pending"),
        consentNote: z2.string().max(1e3).nullable().optional()
      })
    ).mutation(async ({ ctx, input }) => {
      const data = Buffer.from(input.dataBase64, "base64");
      if (data.byteLength !== input.sizeBytes || data.byteLength > MAX_UPLOAD_BYTES) {
        throw new TRPCError3({ code: "BAD_REQUEST", message: "This file is larger than the supported 8 MB limit or was not encoded correctly." });
      }
      const profile = await getPatientProfile(ctx.user.id);
      if (input.patientProfileId && (!profile || profile.id !== input.patientProfileId)) {
        throw new TRPCError3({ code: "FORBIDDEN", message: "That patient profile does not belong to this caregiver." });
      }
      ...
      return insertStoredFile({
        ...
        consentRecordedAt: input.consentStatus === "confirmed" ? /* @__PURE__ */ new Date() : null,
    ...
    update: protectedProcedure.input(z2.object({
      ...
      purpose: z2.enum(uploadPurposes).optional(),
      patientProfileId: z2.number().int().positive().nullable().optional(),
      consentStatus: z2.enum(["pending", "confirmed", "declined"]).optional(),
      consentNote: z2.string().max(1e3).nullable().optional()
      ...
      consentRecordedAt: input.consentStatus === "confirmed" ? /* @__PURE__ */ new Date() : void 0
    })),
    remove: protectedProcedure.input(z2.object({ id: z2.number().int().positive() })).mutation(({ ctx, input }) => removeStoredFile(ctx.user.id, input.id))
  })
});
